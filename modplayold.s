...
valPepPlaying			.byte	$00
valPepGlobalVolume		.byte	$ff					; D729

adrPepMODL:				.word	$0000
adrPepMODH:				.word	$0000
...
valPepTmp0:				.word	$0000
valPepTmp1:				.word	$0000
valPepTmp2:				.word	$0000
valPepTmp3:				.word	$0000
valPepTmp4:				.word	$0000

adrPepPtn0:				.repeat	128
							.dword $00000000
						.endrepeat

idxPepPtn0:				.repeat	128
							.byte 0
						.endrepeat

recPepIns0:				.repeat	32
							.res .sizeOf(PEP_INSDATA)
						.endrepeat

idxPepIns0:				.repeat	32, ins
...
valPepSig0:				.byte	$4D, $2E, $4B, $2E
						.byte	$4D, $21, $4B, $21
						.byte	$46, $4C, $54, $34
						.byte	$46, $4C, $54, $38

valPepSin0:				.byte	0,  24,  49,  74,  97, 120, 141, 161
						.byte	180, 197, 212, 224, 235, 244, 250, 253
						.byte	255, 253, 250, 244, 235, 224, 212, 197
						.byte	180, 161, 141, 120,  97,  74,  49,  24

valPepCurrentSamples:	.byte 0,0,0,0

valPepFTn0:				.word	4340, 4308, 4277, 4247, 4216, 4186, 4156, 4126
...
@ins320:	__PEP_SET_MODF_OFFS ptrPepModF, $03B6
...
@ins321:	__PEP_SET_MODF_OFFS ptrPepMSeq, $03B8
...
@procTick:	JSR	pepptioPerformTick
@exit:		JSR	peppitoRestoreState
			RTS
...
@bytes0:	;	If (loop start / 2) + loop length <= sample length
			;		slen < lstrt / 2 + llen
			CLC
...
@bytes1:	;	loop len:= sample len - loop start
			SEC
			LDA	valPepTmp4
			SBC	valPepTmp0
			STA	valPepTmp1
...
@less0:		;	loop start:= loop start / 2
			LDA	valPepTmp2
			STA	valPepTmp0
			LDA	valPepTmp2 + 1
			STA	valPepTmp0 + 1

@skip0:		;	if LoopLength < 4 then
...
@cont1:		;	Store loop start and loop length
...
@ins320:	__PEP_SET_MODF_OFFS ptrPepModF, $043C
...
@less1:		LDA	valPepTmp2
			STA	valPepTmp1
			LDA	valPepTmp2 + 1
			STA	valPepTmp1 + 1

@cont0:		LDA	valPepTmp2 + 1
			CMP	valPepTmp1 + 1
			BCC	@less2
			BNE	@cont1
			LDA	valPepTmp2
...
@less3:		LDA	valPepTmp2
			STA	valPepTmp1
			LDA	valPepTmp2 + 1
			STA	valPepTmp1 + 1

@cont1:		LDY	#PEP_CHNDATA::valPeriod
			LDA	valPepTmp1
			STA	(ptrPepChan), Y
			INY
			LDA	valPepTmp1 + 1
...
@update:	LDY	#PEP_CHNDATA::valVibAdd
			LDA	valPepTmp2
			STA	(ptrPepChan), Y
...
@retrig0:	LDY	#PEP_CHNDATA::valFxCnt
			LDA	#$00
			STA	(ptrPepChan), Y
...
@trigger:	JSR	peppitoChanTrigger

@update:	JSR	peppitoChanTrigEffect
...
@vibspd0:	LDY	#PEP_CHNDATA::valVibSpd
			STA	(ptrPepChan), Y

@vibcont0:	LDA	valPepTmp0 + 1
			AND	#$0F
...
@vibdep0:	LDY	#PEP_CHNDATA::valVibDep
			STA	(ptrPepChan), Y

@vibcont1:	JSR	peppitoChanVibrato
			JSR	peppitoChanUpdFreq
...
@tport0:	LDY	#PEP_CHNDATA::valPrtSpd
			STA	(ptrPepChan), Y
...
@ptnl2:		LDA	(ptrPepChan), Y
			STA	valPepNRow

@ptnl3:		DEC	cntPepPtnL
			JMP	@exit
...
@cont0:		LDY	#PEP_CHNDATA::valPeriod
			CLC
			LDA	(ptrPepChan), Y
			ADC	valPepTmp1
			STA	valPepTmp1
